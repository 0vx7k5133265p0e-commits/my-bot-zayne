...
async def casino(interaction: discord.Interaction, category: discord.CategoryChannel = None):
    ...
    try:
        ...
        await channel.send(
            ...
            f"ここで `/slot` `/bj` `/janken` `/gacha` `/dive` `/haikou` などのゲームを楽しめます。\n"
...
class ChangeBetModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.game_type == "slot":
            ...
            view = HaikouView(self.user_id, new_bet, floor=1, sanity=100)
            await interaction.message.edit(
                content=(
                    f"🏫 **廃校探索開始！**（賭け金: **{new_bet} pt**）\n"
                    f"・現在の階層: **1階**（倍率: **×1.0**）\n"
                    f"・正気度: **100 / 100**\n\n"
                    f"暗闇の校舎へ足を踏み入れてください👇"
                ),
...
class HaikouPlayAgainView(discord.ui.View):
    ...
    @discord.ui.button(label="🔄 同じ賭け金で再挑戦", style=discord.ButtonStyle.primary)
    async def play_again(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        data = load_data()
        user_info = get_user_data(self.user_id, data)
        ...
        view = HaikouView(self.user_id, self.bet, floor=1, sanity=100)
        await interaction.message.edit(
            content=(
                f"🏫 **廃校探索開始！**（賭け金: **{self.bet} pt**）\n"
                f"・現在の階層: **1階**（倍率: **×1.0**）\n"
                f"・正気度: **100 / 100**\n\n"
                f"暗闇の校舎へ足を踏み入れてください👇"
            ),
...
class HaikouView(discord.ui.View):
    def __init__(self, user_id, bet, floor=1, sanity=100):
        super().__init__(timeout=None)
        self.user_id = str(user_id)
        self.bet = bet
        self.floor = floor
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("⚠️ あなたの探索画面ではありません！", ephemeral=True)
            return False
        return True
    ...
    @discord.ui.button(label="🚪 奥の階へ進む (探索)", style=discord.ButtonStyle.primary)
    async def advance_floor(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        
        self.floor += 1
        sanity_loss = random.randint(10, 25) + int(self.floor * 1.5)
        ...
        if roll < trap_chance:
            event_type = random.choice(["shadow", "trap", "doll"])
            data = load_data()
            user_info = get_user_data(self.user_id, data)
            ...
            if event_type == "shadow":
                ...
                self.stop()
                await interaction.message.edit(
                    content=f"👤 **背後に何者かの気配が…！パニックで全額ロスト（{self.floor}階）**\n**-{self.bet} pt**（所持: **{user_info['points']} pt**）",
                    view=again_view
                )
                return
                ...
                self.stop()
                await interaction.message.edit(
                    content=f"🩸 **床の落とし穴に落ちてしまいました！全没収（{self.floor}階）**\n**-{self.bet} pt**（所持: **{user_info['points']} pt**）",
                    view=again_view
                )
                return
            else:
                if random.random() < 0.4:
                    mult = self.get_haikou_multiplier(self.floor) * 2
                    payout = int(self.bet * mult)
                    profit = payout - self.bet
                    user_info["points"] += profit
                    save_data(data)
                    self.stop()
                    await interaction.message.edit(
                        content=f"✨ **校長室の隠し金庫を発見！大当たりの霊石を手に入れた！（{self.floor}階）**\n🎉 **+{payout} pt 獲得！**（所持: **{user_info['points']} pt**）",
                        view=again_view
                    )
                    return
                else:
                    ...
                    self.stop()
                    await interaction.message.edit(
                        content=f"🕯️ **呪われた人形に呪い殺されました...（{self.floor}階）**\n**-{self.bet} pt**（所持: **{user_info['points']} pt**）",
                        view=again_view
                    )
                    return
    ...
    @discord.ui.button(label="🏃‍♂️ 正門から脱出して配当を回収 (リターン)", style=discord.ButtonStyle.success)
    async def escape_haikou(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        mult = self.get_haikou_multiplier(self.floor)
        payout = int(self.bet * mult)
        profit = payout - self.bet
...
@client.tree.command(name="haikou", description="廃校探索（肝試し）ギャンブルを開始します（専用カジノ部屋限定）")
@app_commands.describe(bet="賭けるポイント数")
async def haikou(interaction: discord.Interaction, bet: int):
    ...
    view = HaikouView(uid, bet, floor=1, sanity=100)
    await interaction.followup.send(
        f"🏫 **廃校探索開始！**\n"
        f"・賭け金: **{bet} pt**\n"
        f"・現在の階層: **1階**（倍率: **×1.0**）\n"
...
@client.tree.command(name="datasave", description="データを手動で保存します（管理者限定）")
